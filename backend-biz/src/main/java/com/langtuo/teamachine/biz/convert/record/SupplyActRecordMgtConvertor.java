...
public class SupplyActRecordMgtConvertor {
    public static List<SupplyActRecordDTO> convertToSupplyActRecordDTO(List<SupplyActRecordPO> poList, boolean fillDetail) {
        if (CollectionUtils.isEmpty(poList)) {
            return null;
        }

        List<SupplyActRecordDTO> list = poList.stream()
                .map(po -> convertToSupplyActRecordDTO(po, fillDetail))
                .collect(Collectors.toList());
        return list;
    }

    public static SupplyActRecordDTO convertToSupplyActRecordDTO(SupplyActRecordPO po, boolean fillDetail) {
        if (po == null) {
            return null;
        }
